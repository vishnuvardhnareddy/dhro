...
const MyCourses = () => {
    ...
    useEffect(() => {
        const fetchMyCourses = async () => {
            ...
            try {
                const response = await fetch("http://localhost:5000/api/courses/my-courses", { // ✅ Fixed API route
                ...
                {myCourses.length > 0 ? (
                    myCourses.map((course) => (
                        <div key={course._id} className="card" onClick={() => handleCourseClick(course._id, course.type)}>
                            <img src={course.imageUrl || "/placeholder.jpg"} alt={course.title} className="card-image" />
